...
const SQS_QUEUE_URL = "https://sqs.us-west-2.amazonaws.com/727348040440/ToCustomerSQSToPolly.fifo";
const SQS_MESSAGE_ID = "test";
const US_WEST_2_REGION = 'us-west-2';
const SQS_API_VERSION = '2012-11-05';
AWS.config.region = US_WEST_2_REGION; // Region
...
function setAWSCredentials(){
    AWS.config.credentials = new AWS.Credentials(access_id,secret_key);
}

function initializeSQSService(){
    sqsService = new AWS.SQS({apiVersion: SQS_API_VERSION, region: AWS.config.region, credentials: AWS.config.credentials});
}
...
function sendMessageToSqs(transcript){
    let params ={
        MessageBody: transcript,
        QueueUrl: SQS_QUEUE_URL,
        MessageGroupId:SQS_MESSAGE_ID,
        MessageDeduplicationId:SQS_MESSAGE_ID + new Date().getTime()
    };
    let d = new Date().getTime();
    sqsService.sendMessage(params, function(err,data){
        if (err) {
            console.log(err,err.stack);
        }
        else {
            let diff = new Date().getTime()-d;
            console.log("Total time to send SQS Message: " + diff);
            console.log(data.MD5OfMessageBody);
        }
    });
}
